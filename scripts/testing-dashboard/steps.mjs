...
export const steps = [
  ...
  },
  {
    ...
    expected:
      "Confirmació en el NOU idioma; els missatges ja enviats no canvien. L'idioma suggerit a cada agent es calcula a partir del que ja tenia detectat al pas 0.1 (taula users) i és sempre diferent del seu; entre els 4 agents es couen els 4 idiomes suportats (en/ca/es/fr) sempre que sigui possible — només no ho és si els 4 ja tenien exactament el mateix idioma de partida.",
    ...
    // one — a permutation of en/ca/es/fr across A/B/C/D when one exists,
    // so all 4 supported languages get exercised across the group. Falls
    // back to a single shared different language only in the degenerate
    // case where all 4 agents already share the same current language
    // (a permutation avoiding everyone's own language is then impossible).
    prepare(baseline, ctx) {
      const LANGS = ['en', 'ca', 'es', 'fr'];
