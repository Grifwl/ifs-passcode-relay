...
export async function handlePromote(ctx: Context, env: Env): Promise<void> {
  ...
  // The new administrator's inactivity clock (see CLAUDE.md
  // "Administrator succession") starts fresh from the handover, same
  // as the other two transfer paths.
  await touchParticipantActivity(env.DB, targetUserId);
