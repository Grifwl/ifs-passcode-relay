...
|---|---|---|---|
| 0.1 | A, B, C, D | Enviar `/start` | Missatge de benvinguda en l'idioma detectat automàticament del client de Telegram (`en`/`ca`/`es`/`fr`/`gl`/`eu`/`pt`/`it`/`de`, o `en` si no és cap d'aquests) |
| 0.2 | A, B, C, D | Enviar `/help` | Llista de comandes disponibles, en el mateix idioma |
| 0.3 | A, B, C, D | Enviar `/language xx`, on `xx` és un idioma diferent del que cada agent ja tenia detectat al pas 0.1 — triant, entre els 4 agents, una combinació de 4 idiomes diferents entre si d'entre els 9 suportats (`en`/`ca`/`es`/`fr`/`gl`/`eu`/`pt`/`it`/`de`) sempre que sigui possible | Confirmació en el **nou** idioma; els missatges ja enviats no canvien |
| 0.4 | A; després A, B, C, D | Repetir `/language` amb un codi no suportat (p. ex. `/language pl`); tot seguit, cadascú estableix amb `/language <codi>` l'idioma amb què vol continuar la resta de les proves | Codi no suportat: missatge d'error / ús, sense canviar l'idioma actual. Idioma final: cadascú pot quedar-se amb el del pas 0.3, tornar al seu original (pas 0.1), o triar-ne un altre — incloent-hi no fer res, per mantenir el del pas 0.3 |
...
| 1.2 | A | Comprovar els botons d'idioma del segon missatge (`EN`/`CA`/`ES`/`FR`/`GL`/`EU`/`PT`/`IT`/`DE`) | Només apareixen els idiomes **diferents** de l'actual d'A (8 botons), tots en una sola fila |
| 1.3 | A | Prémer un dels botons d'idioma (p. ex. `ES`) | S'envia un **nou** parell de missatges (bloc + nota), ara en espanyol, i el botó `ES` ja no hi surt (n'hi ha 7 dels altres idiomes) |
